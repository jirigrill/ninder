...
export const POST: RequestHandler = async ({ request }) => {
    try {
		...
		if (!newSession.initiatorUserId && !newSession.partnerUserId) {
			return json({ error: 'when creating a session, initiator_user_id is required!' }, { status: 400 });
		}

		if(newSession.initiatorUserId && newSession.partnerUserId) {
			return json({ error: 'when joining a session, initiator_user_id cannot be set!' }, { status: 400 });
		}
        ...
        try {
			if (isNewSession) {
                ...
                if (result.rowCount === 0) {
                    return json({ error: 'No available session found with the provided pairing code' }, { status: 404 });
                }
